...
export class GitHubAPI {
   ...
   */
  async makeRequest(url: string, useCache: boolean = true) {
    // Check cache first
   ...
   */
  async getAllIssues(repositories: string[], startDate: Date, endDate: Date) {
   ...
   */
  async getAllPullRequests(repositories: string[], startDate: Date, endDate: Date) {
    const promises = repositories.map((repoPath) => {
      const [owner, repo] = repoPath.split("/");
   ...
   */
  async getAllRepositories(repositories: string[]) {
    const promises = repositories.map((repoPath) => {
      const [owner, repo] = repoPath.split("/");
  ...
  processPRData(prs: any[], startDate: Date, endDate: Date) {
    ...
    prs.forEach((pr) => {
      ...
      const isCopilot = username.toLowerCase().includes("copilot") || pr.title.toLowerCase().includes("pr merged by copilot") || pr.title.toLowerCase().includes("copilot");
